...
vi.mock('./IncidentRow', () => ({
    default: ({
        ...
        ai_description,
        onView,
    }: IncidentData & { onView: (incident: IncidentData) => void }) => (
        <tr data-testid={`incident-row-${id}`}>
            <td>{ai_description}</td>
            <td>
                <button onClick={() => onView({ id, ai_description, status: 'pending', created_at: '2025-01-10T10:00:00', vehicle_incident_images: [] })}>
                    View
                </button>
            </td>
        </tr>
...
const mockIncident: IncidentData = {
    ...
    vehicle_incident_images: [
        ...
        { id: 2, side: 'left', azure_blob_url: 'https://example.com/base.jpg', image_type: 'base' },
    ],
}
